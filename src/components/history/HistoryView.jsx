...
const HistoryView = ({ tickets, users, services, currentUser, deleteTicketHistory }) => {
  ...
  const historicalTickets = useMemo(() => {
    ...
    if (currentUser.role !== 'admin') {
      userTickets = tickets.filter(t => t.servedBy === currentUser.id);
    }
    
    return userTickets
      .filter(t => ['completed', 'cancelled', 'redirected'].includes(t.status))
      .sort((a, b) => {
  ...
  const getStatusInfo = (ticket) => {
    ...
    if (ticket.status === 'redirected') {
        return { icon: <Send className="h-5 w-5 text-yellow-500" />, text: `Redirigido por mí` };
    }
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text("Empresa: TuCompañía S.A.", 14, 30);
