...
const UserForm = ({ onSave, initialData = {}, departments, currentUser, isEdit = false }) => {
  ...
  const [departmentId, setDepartmentId] = useState(initialData.department_id ? initialData.department_id.toString() : '');
  ...
  useEffect(() => {
    if (role === 'admin') {
      ...
      setPermissions(companyAdminModules);
        ...
        setPermissions(['dashboard', 'tickets', 'history']);
    }
  }, [role]);
  ...
  const handleSubmit = () => {
    const userData = {
      username,
      role,
      department_id: role === 'employee' ? parseInt(departmentId, 10) : null,
      permissions,
      photo,
    };
    if (password) {
  ...
  return (
    <div className="py-4 space-y-4">
        ...
        <>
          <div>
            ...
            <Select value={departmentId} onValueChange={setDepartmentId}>
              <SelectTrigger><SelectValue placeholder="Asignar departamento..." /></SelectTrigger>
              <SelectContent>
                {departments.map(d => (
      ...
      <Button onClick={handleSubmit} className="w-full" disabled={!username || (!password && !isEdit) || (role === 'employee' && !departmentId)}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {user.role === 'admin' ? 'Administrador' : `Empleado - ${departments.find(d => d.id === user.department_id)?.name || 'Sin asignar'}`}
                  </p>
                </div>
              </div>
