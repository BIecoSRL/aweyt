...
import { Save, Image as ImageIcon, Video, MessageSquare, Upload, Building, Settings } from 'lucide-react';
  ...
  return (
    <div className="py-4 space-y-6 max-h-[70vh] overflow-y-auto pr-2">
      ...
      <div>
        ...
        <Input id="videoUrl" name="videoUrl" value={config.videoUrl} onChange={handleChange} placeholder="https://example.com/background.mp4"/>
        <p className="text-xs text-slate-500 mt-1">Usa un video corto y en bucle para el mejor rendimiento.</p>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mt-2 flex-wrap">
          {(config.images || []).map((img, index) => (
            <img key={index} src={img} alt={`Imagen ${index+1}`} className="h-20 rounded-md border p-1"/>
          ))}
        </div>
      </div>
