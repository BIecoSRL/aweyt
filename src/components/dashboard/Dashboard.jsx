...
const Dashboard = ({ tickets, departments, isSystemActive, setIsSystemActive, currentUser, onLogout }) => {
  ...
  const waitingTicketsCount = useMemo(() => {
    ...
    return tickets.filter(t => t.status === 'waiting' && t.departmentId === currentUser.departmentId).length;
  }, [tickets, currentUser]);

  const recentTickets = useMemo(() => {
    if (currentUser.role === 'admin') {
      return tickets.slice(-5).reverse();
    }
    return tickets.filter(t => (t.departmentId === currentUser.departmentId && t.status === 'waiting') || t.servedBy === currentUser.id).slice(-5).reverse();
  }, [tickets, currentUser]);
  ...
  if (currentUser.role === 'admin') {
    ...
    statsCards[1].value = tickets.filter(t => t.status === 'completed').length;
    statsCards[3].title = "Tiempo Promedio (Global)";
    const allCompleted = tickets.filter(t => t.status === 'completed' && t.completedAt && t.servedAt);
